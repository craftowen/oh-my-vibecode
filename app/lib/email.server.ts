 ...
 *
 * To switch providers, replace `deliver()` — every caller goes through
...
export async function sendEmail(env: Env, message: EmailMessage): Promise<void> {
  ...
  if (!response.ok) {
    // Never throw into the auth flow: a failed email must not turn a successful
    // signup into an error page. Log it and let the user request a resend.
    console.error(
...
/** Minimal, provider-agnostic HTML wrapper for a one-action email. */
...
<html><body style="margin:0;padding:24px;background:#f6f7f9;font-family:system-ui,-apple-system,'Segoe UI',sans-serif;color:#1c1c22">
  <table role="presentation" style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e6e7eb">
    <tr><td style="padding:28px">
      ...
      <p style="margin:0 0 20px;font-size:14px;line-height:1.6;color:#54555f">${body}</p>
      <a href="${url}" style="display:inline-block;padding:10px 18px;border-radius:8px;background:#6d4aff;color:#fff;font-size:14px;text-decoration:none">${actionLabel}</a>
      <p style="margin:20px 0 0;font-size:12px;line-height:1.6;color:#84858f">If the button does not work, paste this link into your browser:<br>${url}</p>
    </td></tr>
  </table>
</body></html>`;
}
